...
class Api():
    ...
    def executeLongOta(self, ver1, ver2, id, times, n=1, comp=""):
        ...
        if n>1 and times>=0:
            if reg_Oldver!=comp:
                status="success"
                self.save_success_faild(status,id,reg_Oldver)
            else:
                status="faild"
                self.save_success_faild(status,id,comp)
            if times==0:
                print("ssss%s"%times)
                pid = os.getppid()
    ...
    def save_success_faild(self,status,id,ver):
        if status=="status":
            with open(id+status+'.txt','a') as f:
                f.write("id is {} gwsl ota to {} {}".format(id,ver,status))
        else:
            with open(id + status + '.txt', 'a') as f:
                f.write("ver is {} id is {} gwsl ota {}".format(ver,id,status))
